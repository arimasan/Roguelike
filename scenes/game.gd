...
var skills_unlocked:   Dictionary = {}   # skill_id → true
var _st_branch_idx:    int  = 0          # UI カーソル列
var _st_tier_idx:      int  = 0          # UI カーソル行
var _skill_survived_fatal: bool = false  # 不屈: このフロアで使用済みか
var _p_extra_action:       bool = false  # 俊足: 追加行動フラグ
var _double_attack_active: bool = false  # 連続攻撃の再帰防止
...
var _thief_mode:              bool = false   # 泥棒状態（店から未払い商品を持ち出した）
...
const KEY_ACTIONS: Dictionary = {
	...
	"skill_tree": {"label": "スキルツリー",      "default": KEY_S},
...
const BGM := {
	...
	"dorobou":       "res://assets/bgm/dorobou.mp3",
	...
	for i in traps.size():
		...
		match trap["type"]:
			...
			"unequip":
				...
				if not p_weapon.is_empty():
					add_message("%s が弾き飛ばされた！" % SealSystem.display_name(p_weapon))
					var dp: Vector2i = _find_free_drop_pos(p_grid)
					if dp != Vector2i(-1, -1):
						_place_floor_item(p_weapon, dp)
					p_weapon = {}
					unequipped = true
				if not p_shield.is_empty():
					add_message("%s が弾き飛ばされた！" % SealSystem.display_name(p_shield))
					var dp: Vector2i = _find_free_drop_pos(p_grid)
					if dp != Vector2i(-1, -1):
						_place_floor_item(p_shield, dp)
					p_shield = {}
					unequipped = true
				if not p_ring.is_empty():
					add_message("%s が弾き飛ばされた！" % SealSystem.display_name(p_ring))
					var dp: Vector2i = _find_free_drop_pos(p_grid)
					if dp != Vector2i(-1, -1):
						_place_floor_item(p_ring, dp)
					p_ring = {}
					unequipped = true
				if not unequipped:
					add_message("しかし何も装備していなかった。")
		# 壊れ判定
...
func _update_area_bgm() -> void:
	...
	if in_shop != _in_shop_area:
		...
		if in_shop:
			...
			# 店を出た瞬間：未払いアイテムがあれば泥棒発動
			if not _thief_mode and ShopLogic.has_unpaid_items(self):
				ShopLogic.trigger_thief(self)
				target_bgm = "dorobou"
			elif _shop_traded:
...
func _unhandled_input(event: InputEvent) -> void:
	...
	# Ctrl+D: デバッグモードトグル（図鑑内に「生成する」ボタンを出すなど）
	if kc == KEY_D and kev.ctrl_pressed:
		...
		if game_state == "bestiary" and is_instance_valid(_bestiary_layer):
			...
			BestiaryUI.open(self)
		return
	# Ctrl+N: デバッグ用 — 強制フロア移動（階段チェックをスキップ）
	if kc == KEY_N and kev.ctrl_pressed and debug_mode and game_state == "playing":
		add_message("[DEBUG] 強制フロア移動！")
		current_floor += 1
		_start_new_floor()
		return
	...
	match game_state:
		...
		"skill_tree":
			SkillTreeUI.handle_input(self, kc)
		"options":
			if kc == KEY_ESCAPE:
...
func _handle_play_input(kc: int) -> void:
	...
	elif kc == key_bindings.get("skill_tree", KEY_S):    SkillTreeUI.open(self)
...
func _try_pickup() -> void:
	# 店アイテムが足元にあれば未払いで拾う（店を出るとき精算 or 泥棒判定）
	for i in shop_items.size():
		if shop_items[i]["grid_pos"] == p_grid:
			ShopLogic.pickup_shop_item(self, i)
			return
	var fi = _item_at(p_grid)
	if fi == null:
...
func _end_player_turn() -> void:
	...
	if game_state == "dialog":
		return
	# 俊足フラグが立っていれば、このターンは敵を動かさずプレイヤーに追加行動を与える
	if _p_extra_action:
		_p_extra_action = false
		Fov.update(self)
		Fov.sync_entity_visibility(self)
		_refresh_hud()
		return
	# 満腹度
	turn_count += 1
	var eff_hunger_rate: int = HUNGER_RATE * 2 if SkillTree.has(self, "survival_2") else HUNGER_RATE
	if turn_count % eff_hunger_rate == 0:
	...
	# 仲間がMH部屋に入っていたら発動
	_check_companion_mh_trigger()
	# 敵の追加スポーン（通常50ターンに1体、泥棒中は8ターンに1体盗賊番）
	if _thief_mode:
		if turn_count % 8 == 0:
			ShopLogic.spawn_thief_guard(self)
	...
	_update_camera()
	_refresh_hud()
	# 俊足（explore_1）: 10%で追加ターン（敵は動かない、プレイヤーのみ2回行動）
	if SkillTree.has(self, "explore_1") and game_state == "playing" and randf() < 0.05:
		add_message("俊足発動！もう一度動ける！")
		_p_extra_action = true
